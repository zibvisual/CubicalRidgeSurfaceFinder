...
namespace fastmarching
{
    ...
    private: // inner data shared by all versions
        void
        data_inner(const RawFieldView<float> *potential)
        {
    ...
    public:
        ...
        void
        data(const RawFieldView<float> *pot)
        {
            data_inner(pot);
        }
        ...
        void
        data(const RawFieldView<float> *pot, MappingView timeData, MappingView distanceData)
        {
            data_inner(pot);
    ...
    public:
        ...
        std::pair<float, float>
        defaultThresholds(const RawFieldView<float> *potential) const
        {
            if (!potential)
            {
        ...
        /**
         * @brief Returns the id of the last node which was fixed.
         */
        uint64_t stoppedAt() const {
            return m_last_fixed_node;
        }

        /**
         * @brief Returns the max time.
         */
        float
    ...
    protected:
        ...
        const RawFieldView<float> *m_potential;
        Observer m_observer;

        // values generated by march
        uint64_t m_last_fixed_node;
        float m_max_time;
